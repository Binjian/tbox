...
# as a JSON object.
def upload(options):
    request = urllib.Request("https://melpon.org/wandbox/api/compile.json")
    request.add_header("Content-Type", "application/json")
    response = urllib.urlopen(request, json.dumps(options))
...
def main():
    parser = argparse.ArgumentParser(
        description="""Upload a directory to Wandbox (http://melpon.org/wandbox).
    ...
    parser.add_argument(
        "directory",
        type=str,
        help="""A directory to upload to Wandbox.
